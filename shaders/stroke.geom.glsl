...
void main() {
	...
	gl_Position = vec4(v0 + s*td - s*d,	0, 1); uv = vec2(0.0, 0.0); EmitVertex();
	gl_Position = vec4(v0 - s*td - s*d,	0, 1); uv = vec2(0.0, 1.0); EmitVertex();
	...
	gl_Position = vec4(v1 + s*td + s*d,	0, 1); uv = vec2(1.0, 0.0); EmitVertex();
	gl_Position = vec4(v1 - s*td + s*d,	0, 1); uv = vec2(1.0, 1.0); EmitVertex();
